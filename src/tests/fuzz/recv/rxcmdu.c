...
const char *fuzz_origin = "00:10:20:bb:bb:bb";
...
static int save_input(const char *file, struct blob_buf *b)
{
	...
	if (str) {
		...
		fp = fopen(file, "a");
                if (fp) {
                        fprintf(fp, "%s", "ubus call ieee1905 rxcmdu ");
                        fprintf(fp, "'%s'\n", str);
			fclose(fp);
		}
		free(str);
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
	...
	if (!ret) {
		...
		blob_buf_init(&b, 0);
		blobmsg_add_string(&b, "src", fuzz_origin);
		blobmsg_add_string(&b, "ifname", fuzz_ifname);
		blobmsg_add_string(&b, "type", fuzz_cmdutype);
		blobmsg_add_u32(&b, "mid", fuzz_mid);
		btostr((uint8_t *)data, size, fuzz_data);
		blobmsg_add_string(&b, "data", fuzz_data);

		save_input("./rxcmdu.input", &b);

		ret = ubus_invoke(ctx, i1905, "rxcmdu", b.head, NULL, NULL, 2000);
		if (ret) {
			fprintf(stderr, "Error: ubus ieee1905 rxcmdu (err = %s)\n",
				ubus_strerror(ret));
		}
		blob_buf_free(&b);
		free(fuzz_data);
...
#define NUM_MUTATIONS	2
	...
	case 0:
		...
		split_and_write_tlv(data, newsize, min_pol, tlvtypes, seed);
		}
		break;
	case 1:
		{
		...
		split_and_write_tlv(data, newsize, num_pol, tlvtypes, seed);
		}
		break;
	}
